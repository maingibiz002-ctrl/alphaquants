...
@csrf_exempt
def api_sync_trade(request):
    """
    API endpoint for external bots/scripts (e.g., MQL5 or Python automated execution) 
    to post automated trade executions directly into the user's journal.
    """
    if request.method == 'POST':
        try:
            ...
            # Authenticate via UserProfile API token
            profile = UserProfile.objects.filter(api_token=api_token).first()
            if not profile:
                return JsonResponse({'status': 'error', 'message': 'Invalid API Token'}, status=401)
            ...
            strategy_tag = data.get('strategy_tag', 'Automated EA')
            notes = data.get('notes', 'Synced via API')
            ...
            discipline_score = 90 if pnl > 0 else 75
            ai_summary = f"API Trade: {direction} on {instrument}. R:R 1:{rr_ratio}."

            journal_entry = TradeJournal.objects.create(
                user=user,
                instrument=instrument,
                direction=direction,
                entry_price=entry_price,
